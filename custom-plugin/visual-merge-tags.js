(function() {
	...
	tinymce.PluginManager.add("visualMergeTags", function(editor, url) {
	  ...
	  var makeMergeTag = function(tagType) {
		var tag = editor.dom.create("button", {}, mergeTags[tagType]);
		const svgImage = '<img src = "/assets/images/merge-tags/iconMergeTag.svg" alt=""/>';
		tag.insertAdjacentHTML('afterbegin', svgImage);
		tag.classList.add("merge-tag");
		tag.classList.add("topol-special-merge-tag");
	  ...
	  editor.on("BeforeExecCommand", function(event) {
		...
		if (
		  ...
		  var mergeTagElements = editor
			.getBody()
			.querySelectorAll("button.merge-tag");
	  ...
	  editor.on("PreInit", function() {
		...
		editor.on("click", function(event) {
		  if (event.target && event.target.matches("button.merge-tag")) {
			target = event.target;
		...
		editor.on("BeforeSetContent", function(event) {
		  ...
		  if (event.content.includes('href="*|')) {
			return;
		  }
  
		  var content = event.content,
			mergeTagRegex = new RegExp(
			  "\\*\\|([a-zA-Z_]+)(\\?[^\\|]+)?\\|\\*",
			  "g"
			),
			contentOffset = 0;
		  ...
		  for (var match of content.matchAll(mergeTagRegex)) {
			...
			var asHtml = tag.outerHTML,
			  ...
			  suffix = content.substring(
				match.index + contentOffset + match[0].length
			  );
		...
		editor.serializer.addNodeFilter("button", function(nodes) {
		  var i = nodes.length,
			node;
